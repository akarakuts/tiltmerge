...
| **Default language** | en-US (с переводами на 23 языка) |
...
Поддерживается 23 языка (исходники в `translations/*.csv`, паритет ключей проверяется в CI). Авто-определение локали Android при первом запуске.

- `en-US` (по умолчанию), `ru-RU`, `de-DE`, `fr-FR`, `es-ES`, `pt-PT`, `pt-BR`, `it-IT`, `tr-TR`, `pl-PL`, `nl-NL`, `uk-UA`, `cs-CZ`, `sv-SE`, `ja-JP`, `ko-KR`, `zh-CN`, `zh-TW`, `id-ID`, `vi-VN`, `th-TH`, `hi-IN`, `ar-SA`

> Полные локализованные описания витрины (название, краткое и полное описание) готовятся отдельно под каждый рынок — короткие строки UI уже переведены в `translations/`.
